-- Permite pedir resultados sin resaltado, para exportar.
--
-- ts_headline se calcula texto por texto y sobre una página de 25 filas no
-- se nota, pero al exportar una búsqueda entera sí: 5000 filas tardan 6,3
-- segundos con resaltado y 0,2 sin él. En un archivo descargado el
-- resaltado además no aporta nada.

drop function if exists buscar_indice(text, integer, integer, text, integer, integer, integer);
...
create or replace function buscar_indice(
  ...
  p_resaltar   boolean  default true
)
returns table (
...
language sql stable security invoker set search_path = public as $$
  ...
  select p.id, p.tipo, p.numero, p.anio, p.fecha, p.indice,
         case when c.ts is null or not p_resaltar then null
...
revoke all on function buscar_indice(text, integer, integer, text, integer, integer, integer, boolean) from anon;
grant execute on function buscar_indice(text, integer, integer, text, integer, integer, integer, boolean) to authenticated;
